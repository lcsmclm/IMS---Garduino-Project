(function(){
  ...
  var gnomedatalinks = document.querySelectorAll('.statistic-gnome');
  var statcont = document.querySelector('.statistic-detail-container');
  function statDetails(e){
    e.preventDefault();
    ...
    console.log(id);
    statRequest = createRequest();  ///subway car creation
  if(statRequest===null){
    alert('ajax not working');
    return;
  }
  	var url = "/statistics/"+id;
  statRequest.onreadystatechange = statStatus; ///the scheduling for the car
  statRequest.open("GET", url, true); //open tunnel
  statRequest.send(null);
  }

  function statStatus(){
    if(statRequest.readyState === 4) {
      var statdata = statRequest.responseText;
  statcont.innerHTML = statdata;
  test();
  ...
  for(var i = 0; i < gnomedatalinks.length; i++){
    gnomedatalinks[i].addEventListener('click', statDetails, false);
  }
